...
Looking for `Reorganize Vault`? It is retired. It ran a whole reshaping sequence - converting links, collecting attachments, deleting empty folders, repairing paths - and once every other step had moved to the plugins named above, all it still ran was **Fix Incompatible Paths**, below. Run that instead; it does exactly what `Reorganize Vault` did.
