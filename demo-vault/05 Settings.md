...
- `treatAsAttachmentExtensions`
  - extensions that should be treated as attachments even though they are Markdown, such as `.excalidraw.md`. A file listed here is collected into a referencing note's attachment folder like any other attachment, instead of being skipped as a note - which is what a drawing wants, since it belongs to whatever note embeds it. It is also never scanned as a note of its own, so what is written inside it stays exactly as it is - a drawing keeps its embedded-image references in its own format, and rewriting them would stop it rendering. Walked through in [04 Reorganize and convert links](<./04 Reorganize and convert links.md>).
