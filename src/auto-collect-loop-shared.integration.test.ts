 ...
 * @file
 *
 * Shared integration suite for the `shouldCollectAttachmentsAutomatically` setting (GitHub issue #152):
 ...
 *
 * Named `*-shared.integration.test.ts` (per G47): excluded from unit collection/coverage and matched by
 * no `*.desktop`/`*.android` project glob — runs only when imported by a platform entry point.
 */
...
/**
 * Registers the auto-collect endless-loop integration test for the given platform.
 *
 * @param platform - Human-readable platform label used in the test name (e.g. `'desktop'`).
 */
export function registerAutoCollectLoopSuite(platform: string): void {
